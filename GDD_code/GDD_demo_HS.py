...
net_input = (
    get_noise(
        input_depth,
        method,
        (int(msi_np.shape[0] / (2 ** n_layer)), int(msi_np.shape[1] / (2 ** n_layer))),
    )
    .type(dtype)
    .detach()
)
...
def closure(ind_iter):
    ...
    out_rgb_torch = torch.bmm(
        out_HR.squeeze(0).permute(1, 2, 0),
        R_torch.squeeze(0)
        .transpose(1, 0)
        .expand(out_HR.size(2), *R_torch.squeeze(0).transpose(1, 0).size()),
    )
    ...
    if i % show_every == 0:
        out_rgb = np.tensordot(
            R, out_HR.detach().cpu().squeeze().numpy(), axes=((1), (0))
        ).transpose(1, 2, 0)
