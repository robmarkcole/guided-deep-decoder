...
class Downsampler_bicubic(nn.Module):
    def __init__(self, scale_factor):
        ...
        self.bicubic = nn.functional.interpolate(
            scale_factor=scale_factor, mode="bicubic", align_corners=None
        )
...
class Downsampler_ave(nn.Module):
    ...
    def __init__(
        ...
        downsampler = nn.Conv2d(
            n_planes, n_planes, kernel_size=self.kernel.shape, stride=1, padding=0
        )
        downsampler.weight.data[:] = 0
        downsampler.bias.data[:] = 0
